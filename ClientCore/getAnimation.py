...
data, data_cam,data_w, center, new_list, new_list_camera, final_frustum = Extractor('generator').extractPoints()
...
#        new_l = np.array(new_list_camera, dtype=np.float64)[i]
        ...
        center = np.array([float(np.mean(new_[:,0])), float(np.mean(new_[:,1])), float(np.mean(new_[:,2]))], dtype=np.float64)
